...
public class UserTest {
  ...
  public void isValidUser() throws Exception {
    ...
    boolean result = obj1.isValidUser(User.getFileName());
    assertEquals(result, false);
    assertNull(obj1.getApiToken());
    //right username wrong password
    obj1 = new User("admin", "pwd");
    result = obj1.isValidUser(User.getFileName());
    assertEquals(result, false);
    assertNull(obj1.getApiToken());
    //both correct
    obj1 = new User("admin", "password");
    assertEquals(obj1.isValidUser(User.getFileName()), true);
    assertNotNull(obj1.getApiToken());
    // for the overloaded method that checks the token
    assertFalse(User.isValidUser("blablabla", User.getFileName()));
    assertFalse(User.isValidUser(null, User.getFileName()));
    assertTrue(User.isValidUser("YWRtaW4scGFzc3dvcmQ=", User.getFileName()));
  }
  ...
  public void ExceptionisValidUser() {
    ...
    User obj1 = new User("test", "pwd");
    assertFalse(obj1.isValidUser("tetsing"));
