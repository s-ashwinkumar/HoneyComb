...
public class ChangeSecurityGroupInLcFault extends AbstractFault {
  ...
  public ChangeSecurityGroupInLcFault(HashMap<String, String> params) throws
      IOException {
    super(params);
    this.asgName = params.get("asgName");
    this.faultySecurityGroupName = params.get("faultySecurityGroupName");
    logger = new Loggi(faultInstanceId, ChangeSecurityGroupInLcFault.class
        .getName());
  }
  ...
  public void start() throws Exception {
    ...
    logger.log("Injecting fault: Attach new LaunchConfiguration with " +
        "different Security Group to the ASG");
    ...
    if (!ec2RunningInstances.isEmpty()) {
      ...
      // Log the action
      logger.log("Faulty LaunchConfiguration with wrong Security Group " +
          "updated. Terminating instance with id = "
          + instanceToInject.getInstanceId() + " for spawning new instance " +
          "with faulty LC...");

      // Terminate the instance
      ec2Service.terminateInstance(instanceToInject.getInstanceId());

      // Delay for 10 seconds (ASG EC2 Health Check time) for ASG to spawn
      // new faulty instance
