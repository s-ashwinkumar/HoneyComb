 ...
 * Provided wrapper services for various operations which involve SSH'ing
 * into EC2 instances.
 *
 * @author anbinhtran
 */
public interface SshService {
   ...
   * @param sshKeyFilePath the path to a Key pair (.PEM) file for SSH
   *                       authorization
   ...
   *                     when trying to read the Key file or creating the SSH
   *                     connection
   */
  public void executeSshCommands(
      String hostname, String sshUser, String sshKeyFilePath, List<String>
      commands)
      throws IOException;

  /**
   * Execute one Shell command on an instance via SSH. Return the command
   * output.
   *
   * @param hostname       the public hostname of the Instance
   * @param sshUser        the Linux user for SSH login (e.g. "ubuntu")
